...
class SharedVec {
 public:
  ...
  SharedVec(const SharedVec& other)
      : data_(std::make_shared<Vec<T>>(other.data_->view())) {}
  ...
  // operator Vec<T>() const { return Vec<T>(data_->view()); }
  ...
  void MakeUnique() {
    if (!data_.unique()) data_ = std::make_shared<Vec<T>>(data_->view());
  }
