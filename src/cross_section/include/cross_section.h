...
class CrossSection {
 public:
  ...
  static CrossSection Square(const glm::dvec2 dims, bool center = false);
   ...
   */
  ///@{
  CrossSection Translate(const glm::dvec2 v) const;
  CrossSection Rotate(double degrees) const;
  CrossSection Scale(const glm::dvec2 s) const;
  CrossSection Mirror(const glm::dvec2 ax) const;
  CrossSection Transform(const glm::dmat3x2& m) const;
  CrossSection Warp(std::function<void(glm::dvec2&)> warpFunc) const;
 ...
 private:
  ...
  mutable glm::dmat3x2 transform_ = glm::dmat3x2(1.0);
