...
void Manifold::Impl::SimplifyTopology() {
  ...
  scratchBuffer.reserve(10);
  {
    ...
    numFlagged = 0;
    ShortEdge se{halfedge_, vertPos_, uncertainty_};
...

  {
    ...
    numFlagged = 0;
    SwappableEdge se{halfedge_, vertPos_, faceNormal_, uncertainty_};
    for_each_n(policy, countAt(0_uz), nbEdges,
               [&](size_t i) { bFlags[i] = se(i); });
...
void Manifold::Impl::CollapseEdge(const int edge, std::vector<int>& edges) {
  ...
  const bool shortEdge = la::dot(delta, delta) < uncertainty_ * uncertainty_;
...
void Manifold::Impl::RecursiveEdgeSwap(const int edge, int& tag,
  ...
  if (CCW(v[0], v[1], v[2], uncertainty_) > 0 || !Is01Longest(v[0], v[1], v[2]))
    return;
  ...
  if (CCW(v[1], v[0], v[3], uncertainty_) <= 0) {
    ...
    return;
  } else if (CCW(v[0], v[3], v[2], uncertainty_) <= 0 ||
             CCW(v[1], v[2], v[3], uncertainty_) <= 0) {
    return;
  }
  // Normal path
  SwapEdge();
