...
struct InterpTri {
  ...
  void operator()(const int vert) {
    ...
    const mat3x4 corners = {
        impl->vertPos_[halfedge[halfedges[0]].startVert],
        impl->vertPos_[halfedge[halfedges[1]].startVert],
        impl->vertPos_[halfedge[halfedges[2]].startVert],
        halfedges[3] < 0
            ? vec3(0.0)
            : impl->vertPos_[halfedge[halfedges[3]].startVert]};
    ...
    if (halfedges[3] < 0) {  // tri
      ...
      const mat4x3 tangentL = {
          impl->halfedgeTangent_[halfedge[halfedges[2]].pairedHalfedge],
          impl->halfedgeTangent_[halfedge[halfedges[0]].pairedHalfedge],
          impl->halfedgeTangent_[halfedge[halfedges[1]].pairedHalfedge]};
      ...
      const mat4 tangentsY = {
          impl->halfedgeTangent_[halfedge[halfedges[3]].pairedHalfedge],
          impl->halfedgeTangent_[halfedges[1]],
          impl->halfedgeTangent_[halfedge[halfedges[1]].pairedHalfedge],
...
vec3 Manifold::Impl::GetNormal(int halfedge, int normalIdx) const {
  ...
  const int prop = halfedgeView[halfedge].propVert;
  vec3 normal;
  for (const int i : {0, 1, 2}) {
...
vec4 Manifold::Impl::TangentFromNormal(const vec3& normal, int halfedge) const {
  const VecView<const Halfedge> halfedgeView = halfedge_;
  const Halfedge edge = halfedgeView[halfedge];
...
bool Manifold::Impl::IsInsideQuad(int halfedge) const {
  ...
  const int pair = halfedgeView[halfedge].pairedHalfedge;
  const int pairTri = pair / 3;
  const TriRef pairRef = meshRelation_.triRef[pairTri];
  if (!ref.SameFace(pairRef)) return false;

  auto SameFace = [this, halfedgeView](int halfedge, const TriRef& ref) {
    return ref.SameFace(
        meshRelation_.triRef[halfedgeView[halfedge].pairedHalfedge / 3]);
  };
...
Vec<bool> Manifold::Impl::FlatFaces() const {
  ...
  for_each_n(autoPolicy(numTri, 1e5), countAt(0), numTri,
             [this, &triIsFlatFace, halfedge](const int tri) {
               const TriRef& ref = meshRelation_.triRef[tri];
               int faceNeighbors = 0;
               ivec3 faceTris = {-1, -1, -1};
               for (const int j : {0, 1, 2}) {
                 const int neighborTri =
                     halfedge[3 * tri + j].pairedHalfedge / 3;
...
Vec<int> Manifold::Impl::VertFlatFace(const Vec<bool>& flatFaces) const {
  ...
  for (size_t tri = 0; tri < NumTri(); ++tri) {
    if (flatFaces[tri]) {
      for (const int j : {0, 1, 2}) {
        const int vert = halfedge[3 * tri + j].startVert;
...
Vec<int> Manifold::Impl::VertHalfedge() const {
  ...
  for_each_n(autoPolicy(halfedge_.size(), 1e5), countAt(0), halfedge_.size(),
             [&vertHalfedge, &counters, halfedge](const int idx) {
               auto old = std::atomic_exchange(
                   reinterpret_cast<std::atomic<uint8_t>*>(
                       &counters[halfedge[idx].startVert]),
                   static_cast<uint8_t>(1));
               if (old == 1) return;
               // arbitrary, last one wins.
               vertHalfedge[halfedge[idx].startVert] = idx;
             });
  return vertHalfedge;
}
...
std::vector<Smoothness> Manifold::Impl::SharpenEdges(
  ...
  for (size_t e = 0; e < halfedge.size(); ++e) {
    if (!halfedge[e].IsForward()) continue;
    const size_t pair = halfedge[e].pairedHalfedge;
...
void Manifold::Impl::SetNormals(int normalIdx, double minSharpAngle) {
  ...
  for_each_n(autoPolicy(halfedge_.size(), 1e5), countAt(0), halfedge_.size(),
             [halfedge, &oldHalfedgeProp](int i) mutable {
               oldHalfedgeProp[i] = halfedge[i].propVert;
               halfedge[i].propVert = -1;
             });

  const int numEdge = halfedge_.size();
  for (int startEdge = 0; startEdge < numEdge; ++startEdge) {
    if (halfedge[startEdge].propVert >= 0) continue;
    const int vert = halfedge[startEdge].startVert;
    ...
    if (vertNumSharp[vert] < 2) {  // vertex has single normal
      ...
      ForVert(startEdge, [&](int current) {
        ...
        halfedge[current].propVert = prop;
      ...
      do {  // find a sharp edge to start on
        int next = NextHalfedge(halfedge[current].pairedHalfedge);
      ...
      ForVert<FaceEdge>(
          endEdge,
          [this, centerPos, &vertNumSharp, &vertFlatFace, halfedge](
              int current) {
            ...
            if (vertNumSharp[vert] < 2) {
              ...
              pos += vec3(
                  TangentFromNormal(normal, halfedge[current].pairedHalfedge));
            }
      ...
      ForVert(endEdge, [&](int current1) {
        ...
        halfedge[current1].propVert = newProp;
        ++idx;
      });
    }
  }
...
void Manifold::Impl::LinearizeFlatTangents() {
  ...
  const VecView<const Halfedge> halfedge = halfedge_;
  for_each_n(autoPolicy(n, 1e4), countAt(0), n, [this, halfedge](const int halfedgeIdx) {
    vec4& tangent = halfedgeTangent_[halfedgeIdx];
    vec4& otherTangent =
        halfedgeTangent_[halfedge[halfedgeIdx].pairedHalfedge];

    const bool flat[2] = {tangent.w == 0, otherTangent.w == 0};
    if (!halfedge[halfedgeIdx].IsForward() || (!flat[0] && !flat[1])) {
      return;
    }

    const vec3 edgeVec = vertPos_[halfedge[halfedgeIdx].endVert] -
                         vertPos_[halfedge[halfedgeIdx].startVert];
...
void Manifold::Impl::DistributeTangents(const Vec<bool>& fixedHalfedges) {
  ...
  for_each_n(
      ...
      [this, &fixedHalfedges, halfedgeView](int halfedgeIdx) {
        if (!fixedHalfedges[halfedgeIdx]) return;

        if (IsMarkedInsideQuad(halfedgeIdx)) {
          halfedgeIdx = NextHalfedge(halfedgeView[halfedgeIdx].pairedHalfedge);
        }
        ...
        const vec3 approxNormal =
            vertNormal_[halfedgeView[halfedgeIdx].startVert];
        const vec3 center = vertPos_[halfedgeView[halfedgeIdx].startVert];
        vec3 lastEdgeVec = SafeNormalize(
            vertPos_[halfedgeView[halfedgeIdx].endVert] - center);
        const vec3 firstTangent =
            SafeNormalize(vec3(halfedgeTangent_[halfedgeIdx]));
        ...
        do {
          ...
          const vec3 thisEdgeVec =
              SafeNormalize(vertPos_[halfedgeView[current].endVert] - center);
        ...
        double offset = 0;
        if (current == halfedgeIdx) {  // only one - find average offset
...
void Manifold::Impl::CreateTangents(int normalIdx) {
  ...
  for_each_n(
      ...
      [this, &tangent, &fixedHalfedge, normalIdx, halfedge](int e) {
        struct FlatNormal {
          bool isFlatFace;
          vec3 normal;
        };
        ...
        ForVert<FlatNormal>(
            e,
            [normalIdx, this](int halfedgeIdx) {
              const vec3 normal = GetNormal(halfedgeIdx, normalIdx);
              const vec3 diff = faceNormal_[halfedgeIdx / 3] - normal;
              return FlatNormal(
                  {la::dot(diff, diff) < kPrecision * kPrecision, normal});
            },
            [&faceEdges, &tangent, &fixedHalfedge, halfedge, this](
                int halfedgeIdx, const FlatNormal& here,
                const FlatNormal& next) {
              if (IsInsideQuad(halfedgeIdx)) {
                tangent[halfedgeIdx] = {0, 0, 0, -1};
                return;
              }
              // mark special edges
              ...
              if (differentNormals || here.isFlatFace != next.isFlatFace) {
                fixedHalfedge[halfedgeIdx] = true;
                if (faceEdges[0] == -1) {
                  faceEdges[0] = halfedgeIdx;
                } else if (faceEdges[1] == -1) {
                  faceEdges[1] = halfedgeIdx;
                } else {
                  faceEdges[0] = -2;
                }
              }
              ...
              if (differentNormals) {
                ...
                tangent[halfedgeIdx] = CircularTangent(
                    (la::dot(dir, edgeVec) < 0 ? -1.0 : 1.0) * dir, edgeVec);
              } else {
                tangent[halfedgeIdx] =
                    TangentFromNormal(here.normal, halfedgeIdx);
              }
            });

        if (faceEdges[0] >= 0 && faceEdges[1] >= 0) {
          const vec3 edge0 = vertPos_[halfedge[faceEdges[0]].endVert] -
                             vertPos_[halfedge[faceEdges[0]].startVert];
          const vec3 edge1 = vertPos_[halfedge[faceEdges[1]].endVert] -
                             vertPos_[halfedge[faceEdges[1]].startVert];
...
void Manifold::Impl::CreateTangents(std::vector<Smoothness> sharpenedEdges) {
  ...
  for_each_n(autoPolicy(numHalfedge, 1e4), countAt(0), numHalfedge,
             [&tangent, &vertNormal, halfedge, this](const int edgeIdx) {
               tangent[edgeIdx] =
                   IsInsideQuad(edgeIdx)
                       ? vec4(0, 0, 0, -1)
                       : TangentFromNormal(
                             vertNormal[halfedge[edgeIdx].startVert], edgeIdx);
             });
  ...
  for (size_t tri = 0; tri < NumTri(); ++tri) {
    ...
    for (const int j : {0, 1, 2}) {
      const int tri2 = halfedge[3 * tri + j].pairedHalfedge / 3;
  ...
  for (Smoothness edge : sharpenedEdges) {
    ...
    const bool forward = halfedge[edge.halfedge].IsForward();
    const int pair = halfedge[edge.halfedge].pairedHalfedge;
  ...
  for_each_n(
      ...
      [this, &vertTangents, &fixedHalfedge, &vertHalfedge, &triIsFlatFace,
       halfedge](int v) {
        ...
        if (vert.size() == 2) {  // Make continuous edge
          ...
          const vec3 pos = vertPos_[halfedge[first].startVert];
          halfedgeTangent_[first] = CircularTangent(
              newTangent, vertPos_[halfedge[first].endVert] - pos);
          halfedgeTangent_[second] = CircularTangent(
              -newTangent, vertPos_[halfedge[second].endVert] - pos);
          ...
          ForVert(vert[0].first.halfedge,
                  [this, &triIsFlatFace, smoothness, halfedge](int current) {
                    if (!IsMarkedInsideQuad(current)) {
                      const int pair = halfedge[current].pairedHalfedge;
...
void Manifold::Impl::Refine(std::function<int(vec3, vec4, vec4)> edgeDivisions,
                            bool keepInterior) {
  if (IsEmpty()) return;
  // should not copy everything... only vertPos, halfedgeTangent and halfedge
