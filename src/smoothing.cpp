...
bool Manifold::Impl::IsInsideQuad(int halfedge) const {
  ...
  auto SameFace = [this](int halfedge, const TriRef& ref) {
    return ref.SameFace(
        meshRelation_.triRef[halfedge_.Pair(halfedge) / 3]);
  };
      ...
      ForVert<FaceEdge>(
          ...
          [this, centerPos, &vertNumSharp, &vertFlatFace](int current) {
            ...
            if (vertNumSharp[vert] < 2) {
              ...
              pos += vec3(
                  TangentFromNormal(normal, halfedge_.Pair(current)));
            }
      ...
      autoPolicy(numVert, 1e4), vertHalfedge.begin(), numVert, [&](int e) {
        ...
        ForVert<FlatNormal>(
            ...
            [&](int halfedge, const FlatNormal& here, const FlatNormal& next) {
              ...
              if (EqualNormals(next.normal, here.normal)) {
                ...
                const vec3 edgeVec =
                    vertPos_[halfedge_.End(halfedge)] -
        ...
        if (startHalfedge >= 0) {
          ...
          vec3 prevNormal = GetNormal(
              NextHalfedge(halfedge_.Pair(current)), normalIdx);
          do {
            ...
            if (tangent[current].w == kMissingNormal) {
              ...
              if (EqualNormals(prevNormal, nextNormal)) {
                ...
                const vec3 edgeVec =
                    vertPos_[halfedge_.End(current)] -
       ...
       &triIsFlatFace](int v) {
        ...
        if (vert.size() == 2) {  // Make continuous edge
          ...
          halfedgeTangent_[first] = CircularTangent(
              newTangent, vertPos_[halfedge_.End(first)] - pos);
