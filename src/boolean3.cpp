...
std::optional<RayHit> Manifold::Impl::RayCast(vec3 origin,
                                              vec3 endpoint) const {
  // Delegates to RayCastAll since Collider::Collisions visits all
  // AABB-overlapping leaves regardless — there's no early termination
  // to exploit, so the only overhead is a small vector and sort.
  auto hits = RayCastAll(origin, endpoint);
  if (hits.empty()) return std::nullopt;
  return hits.front();
}

std::vector<RayHit> Manifold::Impl::RayCastAll(vec3 origin,
                                               vec3 endpoint) const {
  ZoneScoped;
  if (IsEmpty()) return {};
