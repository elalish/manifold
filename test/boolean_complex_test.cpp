...
TEST(BooleanComplex, SimpleOffset) {
  ...
  for (size_t i = 0; i < seeds.triVerts.size()/3; i++) {
    int eind[3];
    for (int j = 0; j < 3; j++)
  ...
  for (size_t i = 0; i < seeds.NumVert(); i++) {
    vec3 vpos(seeds.vertProperties[3*i+0], seeds.vertProperties[3*i+1], seeds.vertProperties[3*i+2]);
  ...
  for (e_it = edges.begin(); e_it != edges.end(); ++e_it) {
    vec3 ev1 = vec3(seeds.vertProperties[3*e_it->first+0], seeds.vertProperties[3*e_it->first+1], seeds.vertProperties[3*e_it->first+2]);
    vec3 ev2 = vec3(seeds.vertProperties[3*e_it->second+0], seeds.vertProperties[3*e_it->second+1], seeds.vertProperties[3*e_it->second+2]);
    vec3 edge = ev2 - ev1;
  ...
  for (size_t i = 0; i < seeds.NumTri(); i++) {
    ...
    for (int j = 0; j < 3; j++)
      eind[j] = seeds.triVerts[i*3+j];
    vec3 ev1 = vec3(seeds.vertProperties[3*eind[0]+0], seeds.vertProperties[3*eind[0]+1], seeds.vertProperties[3*eind[0]+2]);
    vec3 ev2 = vec3(seeds.vertProperties[3*eind[1]+0], seeds.vertProperties[3*eind[1]+1], seeds.vertProperties[3*eind[1]+2]);
    vec3 ev3 = vec3(seeds.vertProperties[3*eind[2]+0], seeds.vertProperties[3*eind[2]+1], seeds.vertProperties[3*eind[2]+2]);
    vec3 a = ev1 - ev3;
    vec3 b = ev2 - ev3;
