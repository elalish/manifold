...
class Monotones {
 public:
  ...
  Monotones(const Polygons &polys) {
    ...
    //   std::cout << "idx = " << i << ", mesh_idx = " << monotones_[i].mesh_idx
    //             << std::endl;
    // }
  }
 ...
 private:
  ...
  int PositiveExteriorHelper(int v_idx) {
    ...
    for (int i = 0; i < Num_verts(); ++i) {
      if (Vert(i).Processed() &&
          !Left(Vert(i)).Processed()) {  // Rightward active edge
        ...
        // std::cout << x << " <? " << Vert(v_idx).pos.x << std::endl;
        if (x < Vert(v_idx).pos.x && x > best_x) {
          best_x = x;
          helper_idx = i;
        }
      }
...
class Triangulator {
 public:
  Triangulator(const std::vector<VertAdj> &monotones_, int v_idx)
      : monotones_(monotones_) {
...
void TriangulateMonotones(const std::vector<VertAdj> &monotones_,
                          std::vector<TriVerts> &triangles) {
  // make sorted index list to traverse the sweep line.
  std::vector<std::tuple<float, int>> sweep_line;
  for (int i = 0; i < monotones_.size(); ++i) {
    sweep_line.push_back(std::make_tuple(monotones_[i].pos.y, i));
  }
  std::sort(sweep_line.begin(), sweep_line.end());
  std::vector<Triangulator> triangulators;
  for (int i = 0; i < sweep_line.size(); i++) {
    int v_idx = std::get<1>(sweep_line[i]);
    // std::cout << "i = " << i << ", v_idx = " << v_idx
    //           << ", mesh_idx = " << monotones_[v_idx].mesh_idx << std::endl;
    bool found = false;
    for (auto &triangulator : triangulators) {
      if (triangulator.ProcessVert(v_idx, triangles)) {
        found = true;
        break;
      }
    }
    if (!found) triangulators.emplace_back(monotones_, v_idx);
  }
  // quick validation
...
Polygons Assemble(const std::vector<EdgeVerts> &halfedges) {
  ...
  for (;;) {
    ...
    ALWAYS_ASSERT(result != vert_edge.end(), logicErr, "nonmanifold edge");
...
void CheckManifold(const std::vector<EdgeVerts> &halfedges) {
  ...
  std::vector<EdgeVerts> forward(n_edges), backward(n_edges);
