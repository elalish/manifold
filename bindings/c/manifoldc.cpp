...
namespace {
ManifoldMeshGL *level_set(void *mem, double (*sdf)(double, double, double),
  ...
  std::function<double(glm::dvec3)> fun = [sdf](glm::dvec3 v) {
    return (sdf(v.x, v.y, v.z));
  };
  auto mesh = LevelSet(fun, *from_c(bounds), edge_length, level, !seq);
  return to_c(new (mem) MeshGL(mesh));
}
ManifoldMeshGL *level_set_context(void *mem,
                                  double (*sdf_context)(double, double, double,
                                                        void *),
                                  ManifoldBox *bounds, double edge_length,
                                  double level, bool seq, void *ctx) {
...
ManifoldSimplePolygon *manifold_simple_polygon(void *mem, ManifoldVec2 *ps,
                                               size_t length) {
  auto vec = new (mem) std::vector<glm::dvec2>;
...
ManifoldManifoldPair manifold_split_by_plane(void *mem_first, void *mem_second,
  ...
  auto normal = glm::dvec3(normal_x, normal_y, normal_z);
...
ManifoldManifold *manifold_hull_pts(void *mem, ManifoldVec3 *ps,
                                    size_t length) {
  std::vector<glm::dvec3> vec(length);
  for (int i = 0; i < length; ++i) {
...
ManifoldManifold *manifold_transform(void *mem, ManifoldManifold *m, double x1,
  ...
  auto mat = glm::dmat4x3(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4);
...
ManifoldManifold *manifold_warp(void *mem, ManifoldManifold *m,
  ...
  std::function<void(glm::dvec3 & v)> warp = [fun](glm::dvec3 &v) {
...
ManifoldMeshGL *manifold_level_set_context(
    ...
    ManifoldBox *bounds, double edge_length, double level, void *ctx) {
...
ManifoldMeshGL *manifold_meshgl(void *mem, double *vert_props, size_t n_verts,
...
ManifoldMeshGL *manifold_meshgl_w_tangents(void *mem, double *vert_props,
...
ManifoldManifold *manifold_extrude(void *mem, ManifoldCrossSection *cs,
                                   ...
                                   double scale_y) {
  auto scale = glm::dvec2(scale_x, scale_y);
...
double *manifold_meshgl_vert_properties(void *mem, ManifoldMeshGL *m) {
...
double *manifold_meshgl_run_transform(void *mem, ManifoldMeshGL *m) {
...
double *manifold_meshgl_halfedge_tangent(void *mem, ManifoldMeshGL *m) {
...
ManifoldManifold *manifold_set_properties(void *mem, ManifoldManifold *m,
                                          int num_prop,
                                          void (*fun)(double *new_prop,
                                                      ManifoldVec3 position,
                                                      const double *old_prop)) {
  std::function<void(double *, glm::dvec3, const double *)> f =
      [fun](double *new_prop, glm::dvec3 v, const double *old_prop) {
