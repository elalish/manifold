...
function extract(zipPath: string, destDir: string): void {
  ...
  for (const entry of readEntries(buf)) {
    ...
    else throw new Error(`unsupported compression in ${entry.name}`);
...
function findFolder(root: string, name: string): string | null {
  ...
  while (queue.length) {
    ...
    const kids = readdirSync(dir, { withFileTypes: true }).filter((d) =>
      d.isDirectory(),
    );
    for (const kid of kids) if (kid.name === name) return join(dir, kid.name);
