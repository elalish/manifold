...
__result_items.push((() => {
  const __items = [];
  {
    const __iter_0: any = [1, 2, 3];
    for (const a of __iter_0) {
      __items.push(((overlap: any) => (__apply_color(__extrude(__safe_project3d(Manifold.union([]).translate([0, 0, __mul(__neg(30), a)])), __add(2, overlap)).translate([0, 0, __sub(__mul(2, (__sub(a, 1))), overlap)]), [__add(__div(a, 6), 0.5), 0, 0], undefined)))((__eq(a, 1) ? 0 : 0.1)));
    }
  }
