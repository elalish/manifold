...
function runOpenscadOnce(absFile: string): number {
  ...
  try {
    ...
    execFileSync(
        'taskset', ['-c', '0', 'openscad', ...openscadArgs], {stdio: 'ignore'});
...
// the file at all
function benchmarkFile(absFile: string, openscadAvailable: boolean): FileResult|
    SkippedFile {
  ...
  if (compileTimes.length === RUNS) {
    for (let i = 0; i < RUNS; i++) {
      try {
        const t0 = performance.now();
        execFileSync(
            process.execPath, ['--import', 'tsx', outTs], {stdio: 'ignore'});
