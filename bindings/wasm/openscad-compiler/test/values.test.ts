...
const matchFiles = getAllFiles("./examples/values").filter(f => f.endsWith(".scad"));

const tsForMatch = (scadPath: string) => path.join("out/values", `${path.basename(scadPath, ".scad")}.ts`);
