...
async function childMain() {
  ...
  const runtimeUrl = pathToFileURL(path.join(CWD, "runtime", "runtime.js")).href;
  ...
  try {
    ...
    // Logical Part: The generated file's top-level body runs here. Module and function evaluation, for-loops, list comprehensions, BOSL2 path and vnf math, and the construction of the Manifold operation DAG
    const mod: any = await import(moduleUrl);
    const t2 = performance.now();

    // Geometry Part: Force the DAG to flush, result may be a Manifold(3D) or a CrossSection (2D)
...
interface Stat { mean: number; median: number; min: number; max: number; }
...
// Geometry is a difference of two independently-measured medians, so it can go slightly negative on files where the mesh work is near zero. Clamp for display.
...
interface SkippedFile { file: string; reason: string; }
function isSkipped(r: PhaseResult | SkippedFile): r is SkippedFile { return "reason" in r; }
...
const HAS_TASKSET = (() => {
  try { execFileSync("taskset", ["--version"], { stdio: "ignore" }); return true; }
  catch { return false; }
})();
...
function runOpenscadOnce(absFile: string, ext: "csg" | "off"): number {
  ...
  try {
    ...
    const env = {
      ...process.env,
      OPENSCADPATH: [...libraryPaths, process.env.OPENSCADPATH ?? ""].filter(Boolean).join(path.delimiter),
    };
    
    const args = ["-o", out, `--export-format=${ext}`, "--backend=manifold", absFile];
    const t0 = performance.now();
    if (HAS_TASKSET) execFileSync("taskset", ["-c", "0", "openscad", ...args], { stdio: "ignore", env });
    else execFileSync("openscad", args, { stdio: "ignore", env });
...
function findScadFiles(dir: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) out = out.concat(findScadFiles(full));
    else if (entry.isFile() && full.endsWith(".scad")) out.push(full);
  }
  return out;
}

function benchmarkFile(absFile: string, openscadAvailable: boolean): PhaseResult | SkippedFile {
  ...
  if (!fs.existsSync(outTs)) {
    try {
      ...
      return { file: rel, reason: `transpile: ${(err as Error).message.split("\n")[0]}` };
    }
  }
  ...
  if (openscadAvailable) {
    for (let i = 0; i < RUNS; i++) {
      try { csgTimes.push(runOpenscadOnce(absFile, "csg")); }
      catch (err) { return { file: rel, reason: `openscad csg: ${(err as Error).message.split("\n")[0]}` }; }
    }
    for (let i = 0; i < RUNS; i++) {
      try { offTimes.push(runOpenscadOnce(absFile, "off")); }
      catch (err) { return { file: rel, reason: `openscad off: ${(err as Error).message.split("\n")[0]}` }; }
    }
  }
  ...
  const logicRatio = openscadLogic && ourLogic && ourLogic.median > 0
    ? openscadLogic.median / ourLogic.median : null;
  const geometryRatio = openscadGeometry !== null && ourGeometry && ourGeometry.median > 0
    ? openscadGeometry / ourGeometry.median : null;
  const totalRatio = openscadFull && ourLogic && ourGeometry
    ? openscadFull.median / (ourLogic.median + ourGeometry.median) : null;

  return {
    file: rel,
    openscadLogic, openscadFull, openscadGeometry,
    ourInit, ourLogic, ourGeometry,
    logicRatio, geometryRatio, totalRatio,
    error: errors.length ? errors.join(" | ") : undefined,
  };
}

function writeJson(results: PhaseResult[], skipped: SkippedFile[], openscadAvailable: boolean) {
  ...
  fs.writeFileSync(`${RESULTS_PREFIX}.json`, JSON.stringify({
    ...
    note: "Transpilation excluded by design. init is measured and excluded from logic.",
...
function writeCsv(results: PhaseResult[]) {
  ...
  const header = [
    "file",
    "openscad_logic_median", "openscad_geometry_median", "openscad_total_median",
    "our_init_median", "our_logic_median", "our_geometry_median", "our_total_median",
    "logic_ratio", "geometry_ratio", "total_ratio", "error",
  ].join(",");
  const lines = results.map(r => {
    const ourTotal = r.ourLogic && r.ourGeometry ? r.ourLogic.median + r.ourGeometry.median : null;
    return [
      r.file,
      n(r.openscadLogic?.median), n(r.openscadGeometry), n(r.openscadFull?.median),
      n(r.ourInit?.median), n(r.ourLogic?.median), n(r.ourGeometry?.median), n(ourTotal),
      n(r.logicRatio), n(r.geometryRatio), n(r.totalRatio),
...
function writeMarkdown(results: PhaseResult[], skipped: SkippedFile[], openscadAvailable: boolean) {
  const f = (v: number | null | undefined) => (v === null || v === undefined ? "ERR" : v.toFixed(1));
  ...
  const lines = [
    ...
    "",
    `Runs per file: ${RUNS} · OpenSCAD available: ${openscadAvailable ? "yes" : "no"} · Cores pinned: ${HAS_TASKSET ? "yes" : "no"}`,
    `Examples: \`${path.relative(CWD, EXAMPLES_DIR).replace(/\\/g, "/")}\` · Benchmarked: ${results.length} · Skipped: ${skipped.length}`,
    "",
    "## Method",
    "",
    ...
    "|---|---:|---:|---:|---:|---:|---:|---:|",
    ...results.map(r =>
      `| ${r.file} | ${f(r.openscadLogic?.median)} | ${f(r.openscadGeometry)} | `
      + `${f(r.ourLogic?.median)} | ${f(r.ourGeometry?.median)} | `
      + `${x(r.logicRatio)} | ${x(r.geometryRatio)} | ${x(r.totalRatio)} | `),
    "",
    "",
    `Runtime init overhead (excluded above): median `
    + `${f(results.length ? computeStat(results.flatMap(r => r.ourInit ? [r.ourInit.median] : [])).median : null)} ms per process.`,
  ];

  if (skipped.length) {
    lines.push("", "## Skipped", "", "| File | Reason |", "|---|---|",
...
function printTable(results: PhaseResult[]) {
  ...
  const header = `${"file".padEnd(w)}  ${"os-logic".padStart(9)}  ${"os-geom".padStart(9)}  `
    + `${"our-logic".padStart(9)}  ${"our-geom".padStart(9)}  ${"logic".padStart(7)}  ${"geom".padStart(7)}`;
  console.log(`\n--- Median timings (ms) over ${RUNS} runs ---`);
  console.log(header);
  console.log("-".repeat(header.length));
  for (const r of results) {
    console.log(`${r.file.padEnd(w)}  ${f(r.openscadLogic?.median).padStart(9)}  `
      + `${f(r.openscadGeometry).padStart(9)}  ${f(r.ourLogic?.median).padStart(9)}  `
      + `${f(r.ourGeometry?.median).padStart(9)}  ${x(r.logicRatio).padStart(7)}  ${x(r.geometryRatio).padStart(7)}`);
  }
}
...
function parentMain() {
  ...
  if (!HAS_TASKSET) console.log("taskset unavailable - OpenSCAD will use all cores; ratios will favour it.");
  ...
  files.forEach((absFile, i) => {
    ...
    if (isSkipped(r)) {
      ...
      results.push(r);
      console.log(r.error ? `FAILED (${r.error.slice(0, 90)})` : `ok  logic ${r.logicRatio?.toFixed(2) ?? "?"}x`);
    }
  });
  ...
  const ratios = results.flatMap(r => (r.logicRatio !== null ? [r.logicRatio] : []));
  if (ratios.length) {
    console.log(`Logic-phase speedup across ${ratios.length} files: median ${computeStat(ratios).median.toFixed(2)}x`);
  }
