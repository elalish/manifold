...
import type {Argument, Expr, ForVariable, LetAssignment, ListCompGenerator, Parameter, Program, Statement,} from '../core/ast.js';
...
function checkRange(
    source: string, range: SourceRange, nodeKind: string,
    parentRange?: SourceRange): Violation[] {
  ...
  if (start.offset > end.offset) {
    violations.push({
      nodeKind,
      problem: `start.offset (${start.offset}) > end.offset (${end.offset})`,
      loc: range
    });
  }
  ...
  if (sliced.length === 0 && start.offset !== end.offset) {
    violations.push({
      nodeKind,
      problem: 'empty slice despite non-zero range',
      loc: range,
      sliced
    });
  }
  ...
  if (start.offset > source.length) {
    violations.push({
      nodeKind,
      problem: `start.offset (${start.offset}) past source length (${
          source.length})`,
      loc: range
    });
  }
  if (end.offset > source.length) {
    violations.push({
      nodeKind,
      problem:
          `end.offset (${end.offset}) past source length (${source.length})`,
      loc: range
    });
  }
  ...
  if (computedStart.line !== start.line) {
    violations.push({
      nodeKind,
      problem: `start.line is ${start.line} but offset ${
          start.offset} is on line ${computedStart.line}`,
      loc: range
    });
  }
  if (computedStart.column !== start.column) {
    violations.push({
      nodeKind,
      problem: `start.column is ${start.column} but offset ${
          start.offset} is column ${computedStart.column}`,
      loc: range
    });
  }
  ...
  if (computedEnd.line !== end.line) {
    violations.push({
      nodeKind,
      problem: `end.line is ${end.line} but offset ${end.offset} is on line ${
          computedEnd.line}`,
      loc: range
    });
  }
  if (computedEnd.column !== end.column) {
    violations.push({
      nodeKind,
      problem: `end.column is ${end.column} but offset ${
          end.offset} is column ${computedEnd.column}`,
      loc: range
    });
  }
  ...
  if (parentRange) {
    if (start.offset < parentRange.start.offset) {
      violations.push({
        nodeKind,
        problem: `start.offset (${start.offset}) is before parent start (${
            parentRange.start.offset})`,
        loc: range
      });
    }
    if (end.offset > parentRange.end.offset) {
      violations.push({
        nodeKind,
        problem: `end.offset (${end.offset}) is after parent end (${
            parentRange.end.offset})`,
        loc: range
      });
    }
  }
...
function walkExpr(
    source: string, expr: Expr, violations: Violation[],
    parentRange?: SourceRange): void {
  const range = expr.loc;
  if (range)
    violations.push(...checkRange(source, range, expr.kind, parentRange));
  ...
  switch (expr.kind) {
    ...
    case 'let':
      for (const a of expr.assignments)
        walkLetAssignment(source, a, violations, r);
    ...
    default: {
      ...
      violations.push({
        nodeKind: 'unknown-expr',
        problem: `unhandled expr kind: ${(_never as any).kind}`,
        loc: (expr as any).loc
      });
    }
  }
...
function walkArgument(
    source: string, arg: Argument, violations: Violation[],
    parentRange?: SourceRange): void {
  if (arg.loc)
    violations.push(...checkRange(source, arg.loc, 'argument', parentRange));
  const r = arg.loc ?? parentRange;
  walkExpr(source, arg.value, violations, r);
}

function walkParameter(
    source: string, param: Parameter, violations: Violation[],
    parentRange?: SourceRange): void {
  if (param.loc)
    violations.push(...checkRange(source, param.loc, 'parameter', parentRange));
  const r = param.loc ?? parentRange;
  if (param.defaultValue) walkExpr(source, param.defaultValue, violations, r);
}

function walkLetAssignment(
    source: string, la: LetAssignment, violations: Violation[],
    parentRange?: SourceRange): void {
  if (la.loc)
    violations.push(
        ...checkRange(source, la.loc, 'letAssignment', parentRange));
  const r = la.loc ?? parentRange;
  walkExpr(source, la.value, violations, r);
}

function walkForVariable(
    source: string, fv: ForVariable, violations: Violation[],
    parentRange?: SourceRange): void {
  if (fv.loc)
    violations.push(...checkRange(source, fv.loc, 'forVariable', parentRange));
  const r = fv.loc ?? parentRange;
  walkExpr(source, fv.range, violations, r);
}

function walkListCompGenerator(
    source: string, gen: ListCompGenerator, violations: Violation[],
    parentRange?: SourceRange): void {
  if (gen.loc)
    violations.push(...checkRange(source, gen.loc, gen.kind, parentRange));
  ...
  switch (gen.kind) {
    ...
    case 'lcIf':
      ...
      if (gen.ifFalse)
        walkListCompGenerator(source, gen.ifFalse, violations, r);
      break;
    case 'lcLet':
...
function walkStatement(
    source: string, stmt: Statement, violations: Violation[],
    parentRange?: SourceRange): void {
  if (stmt.loc)
    violations.push(...checkRange(source, stmt.loc, stmt.kind, parentRange));
  ...
  switch (stmt.kind) {
    ...
    default: {
      ...
      violations.push({
        nodeKind: 'unknown-stmt',
        problem: `unhandled stmt kind: ${(_never as any).kind}`,
        loc: (stmt as any).loc
      });
    }
  }
...
function walkProgram(source: string, program: Program): Violation[] {
  const violations: Violation[] = [];
  if (program.loc)
    violations.push(...checkRange(source, program.loc, 'program'));
...
function formatViolation(
    source: string, v: Violation, filename: string): string {
        ...
        if (violations.length > 0) {
          const messages =
              violations
                  .map(
                      v => formatViolation(
                          source, v, path.relative(process.cwd(), file)))
                  .join('\n\n');
          expect.fail(`${violations.length} source-location violation(s) in ${
