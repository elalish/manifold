...
const insertMetaData = (text: string, sourceUrl?: string) => {
  if (sourceUrl) {
    const st = new MagicString(text);
    st.prepend(
      `const _import_meta_url=_manifold_runtime_url ?? '${sourceUrl}';\n`,
    );
 ...
 */
export const esbuildManifoldPlugin = (
  ...
  async setup(build) {
    ...
    if (isNode()) {
      ...
      (async () => {
        ...
        const dir =
          ('string' == typeof __dirname && __dirname) ||
    ...
    build.onResolve({ filter: /.*/ }, async (args) => {
      ...
      const pluginData = {
        toplevel:
          ...
          args.importer === options.filename ||
          args.importer === '<stdin>',
      };
      ...
      if (result.errors.length === 0) {
        ...
        result.pluginData = {
          resolveDir: args.resolveDir.endsWith('/')
            ? args.resolveDir
            : `${args.resolveDir}/`,
        };
        return result;
...
const getEsbuildConfig = async (
  ...
  return {
    ...
    format: 'cjs',
    logLevel: 'silent',
    plugins: [esbuildManifoldPlugin(options)],
