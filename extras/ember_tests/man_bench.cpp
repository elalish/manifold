 ...
 */
static void record_time(const std::string &file1,
                        const std::string &file2,
                        time_point start,
                        time_point before_boolean,
                        time_point after_boolean,
                        time_point end
                    )
{
    ...
    if (!std::filesystem::exists(benchmark_filename)) {
        ...
        stats_file <<
          "file1,file2,total (ms),to manifold (ms),boolean (ms),from manifold (ms)";
    #ifdef CONTROL_PARALLELISM
        stats_file << ",threads";
    #endif
        stats_file << std::endl;
    ...
    stats_file << file1 << "," << file2 << ","
      << std::fixed << std::setprecision(3)
      << total_ms.count() << ","
      << convert_in_ms.count() << ","
      << manifold_boolean_ms.count() << ","
 ...
 */
static MeshGL do_boolean(const std::string &file1,
                         const std::string &file2,
                         const mat3x4 &transform1,
                         const mat3x4 &transform2)
{
...
static void usage()
{
    std::cout << "Usage: %s file1 file2 [-t1 <12 floats>] [-t2 <12 floats>] [--threads <int>]\n";
}
...
int main(int argc, const char **argv) {
    ...
    tbb::global_control control(tbb::global_control::parameter::max_allowed_parallelism,threads);
#endif
