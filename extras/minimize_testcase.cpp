...
inline double cross(glm::dvec2 p, glm::dvec2 q) {
...
inline bool intersect(glm::dvec2 p0, glm::dvec2 p1, glm::dvec2 q0,
                      glm::dvec2 q1, double precision) {
  glm::dvec2 r = p1 - p0;
  glm::dvec2 s = q1 - q0;
  ...
  if (epsilon_r <= u && u <= (1 - epsilon_r) &&  //
    ...
    glm::dvec2 r_orth = glm::normalize(glm::dvec2(-r.y, r.x)) * precision;
...
bool safeToRemove(const Polygons &polys, int i, int j, double precision) {
  if (polys[i].size() == 3) return false;
  glm::dvec2 prev = polys[i][j == 0 ? polys[i].size() - 1 : (j - 1)];
  glm::dvec2 next = polys[i][j == (polys[i].size() - 1) ? 0 : (j + 1)];
  for (int k = 0; k < polys.size(); k++) {
    auto ll = [&](size_t l) {
      return l == (polys[k].size() - 1) ? 0 : (l + 1);
    };
    const glm::dvec2 *polysk = polys[k].data();
    if (!std::all_of(
#if MANIFOLD_PAR == 'T' && __has_include(<pstl/glue_execution_defs.h>)
            std::execution::par,
#endif
...
void DumpTriangulation(const Polygons &polys, double precision) {
  ...
  for (const glm::ivec3 &tri : result) {
    ...
    glm::dvec2 x = polys[xid.first][xid.second];
    glm::dvec2 y = polys[yid.first][yid.second];
    glm::dvec2 z = polys[zid.first][zid.second];
...
bool rayHit(glm::dvec2 point, glm::dvec2 q0, glm::dvec2 q1) {
...
struct Edge {
  glm::dvec2 west;  // -x side vertex
  glm::dvec2 east;  // +x side vertex
