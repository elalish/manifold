...
Manifold bond(int fn, dvec3 p1 = {0, 0, 0}, dvec3 p2 = {1, 1, 1},
...
Manifold bondPair(int fn, double d = 0.0, double ar = 1.0) {
  ...
  dvec3 p1 = {+axD, -axD, -axD};
  dvec3 p2 = {-axD, +axD, +axD};
  Manifold sphere = Manifold::Sphere(ar, fn);
  return sphere.Translate(p1) + sphere.Translate(p2) + bond(fn, p1, p2, ar, ar);
}

Manifold hexagonalClosePacked(int fn, dvec3 dst = {1.0, 1.0, 1.0},
                              double ar = 1.0) {
  std::vector<Manifold> parts;
  dvec3 p1 = {0, 0, 0};
  parts.push_back(Manifold::Sphere(ar, fn));
  double baseAg = 30;
  dvec3 ag = {baseAg, baseAg + 120, baseAg + 240};
  dvec3 points[] = {{cosd(ag.x) * dst.x, sind(ag.x) * dst.x, 0},
...
Manifold fccDiamond(int fn, double ar = 1.0, double unitCell = 2.0,
  ...
  dvec3 corners[] = {{+huc, +huc, +huc},
                     {+huc, -huc, -huc},
                     {-huc, +huc, -huc},
                     {-huc, -huc, +huc}};
  dvec3 fcc[] = {{+huc, 0, 0}, {-huc, 0, 0}, {0, +huc, 0},
  ...
  dvec3 bonds[][2] = {{interstitial[0], corners[0]}, {interstitial[0], fcc[0]},
...
Manifold GraphiteCell(int fn, dvec3 xyz = {1.0, 1.0, 1.0}) {
  dvec3 loc = {(cellLenA * xyz.x * cosd(30) * 2),
